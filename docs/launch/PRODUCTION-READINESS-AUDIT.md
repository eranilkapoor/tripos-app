...
| `tripos-admin-crm`      | Next.js CRM connects to production APIs with auth, organization/branch context, module navigation, listing/search/sort/pagination, forms/actions, invoice builder/export, import/export patterns, permissions, audit logs, storage registry, batch job status, integration health, reporting, notifications, and travel SaaS styling.                                                                                                                                                                                            | Desktop/tablet QA, final role-policy sign-off, custom manual-run button for batch jobs if operations wants it in CRM. |

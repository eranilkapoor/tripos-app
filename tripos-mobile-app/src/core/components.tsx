...
export function Field({ label, onChange, secure, value }: { label: string; onChange: (value: string) => void; secure?: boolean; value: string }) {
...
export function ListScreen({ empty, records, title }: { empty: string; records: ApiRecord[]; title: string }) {
  return <Section title={title}>{records.length ? records.map((record) => <RecordCard key={String(record._id ?? JSON.stringify(record))} record={record} />) : <Text style={styles.body}>{empty}</Text>}</Section>;
}

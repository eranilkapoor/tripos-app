...
import { ApiRecord, MobileSession, loadRecords, logout } from "../api/triposApi";
...
import { AgentHomeScreen, AgentLeadsScreen, AgentQuotesScreen } from "../features/Agent/AgentHomeScreen";
import { LoginScreen } from "../features/Auth/LoginScreen";
import { CustomerDocumentsScreen, CustomerHomeScreen, CustomerTripsScreen } from "../features/Customer/CustomerHomeScreen";
...
export function RootNavigator({ initialSession }: { initialSession: MobileSession | null }) {
  ...
  if (!session) return <LoginScreen onLogin={(nextSession) => void handleLogin(nextSession)} />;

  const bookings = records.bookings ?? [];
  const activeTrip = bookings[0];
  const screenProps: AppScreenProps = { activeTrip, mode, onRefresh: refresh, records, session };
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.appHeader}>
        ...
        <TouchableOpacity style={styles.logoutButton} onPress={() => void handleLogout()}><Text style={styles.logoutText}>Logout</Text></TouchableOpacity>
      </View>

      <View style={styles.segment}>
        <SegmentButton active={mode === "customer"} label="Customer" onPress={() => { setMode("customer"); setTab("home"); }} />
        <SegmentButton active={mode === "agent"} label="Agent" onPress={() => { setMode("agent"); setTab("home"); }} />
      </View>

      <ScrollView contentContainerStyle={styles.container}>
        {tab === "home" && (mode === "customer" ? <CustomerHomeScreen {...screenProps} /> : <AgentHomeScreen {...screenProps} />)}
        {tab === "work" && (mode === "customer" ? <CustomerTripsScreen {...screenProps} /> : <AgentLeadsScreen {...screenProps} />)}
        {tab === "documents" && (mode === "customer" ? <CustomerDocumentsScreen {...screenProps} /> : <AgentQuotesScreen {...screenProps} />)}
      ...
      <View style={styles.tabs}>
        ...
        <TabButton active={tab === "work"} label={mode === "customer" ? "Trips" : "Leads"} onPress={() => setTab("work")} />
        <TabButton active={tab === "documents"} label={mode === "customer" ? "Docs" : "Quotes"} onPress={() => setTab("documents")} />

    ...
    <>
      <View style={styles.heroCard}>
        ...
        <Text style={sharedStyles.body}>
          Track itinerary, documents, payments, vouchers, and support from one
          app.
        </Text>
        <TouchableOpacity
      ...
      <View style={styles.grid}>
        ...
        <Metric
          label="Docs"
          value={String(
            (records["travel-documents"]?.length ?? 0) +
              (records.vouchers?.length ?? 0),
          )}
        />
        <Metric
          label="Payments"
          value={String(records.payments?.length ?? 0)}
        />
      </View>
    </>
  );

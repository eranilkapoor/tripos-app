  ...
  return (
    <>
      <View style={styles.heroCard}>
        ...
        <Text style={sharedStyles.body}>Follow up leads, quotes, bookings, payments, and customer support work.</Text>
      ...
      <View style={styles.grid}>
        ...
        <Metric label="Quotes" value={String(records.quotations?.length ?? 0)} />
        <Metric label="Bookings" value={String(records.bookings?.length ?? 0)} />
      </View>
    </>
  );
}
...
export function AgentLeadsScreen({ records }: AppScreenProps) {
  return <ListScreen empty="No assigned leads yet" records={records.leads ?? []} title="Assigned Leads" />;
}

export function AgentQuotesScreen({ records }: AppScreenProps) {
  return <ListScreen empty="No quotations yet" records={records.quotations ?? []} title="Quotations" />;
}
